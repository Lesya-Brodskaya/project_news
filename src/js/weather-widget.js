...
async function fetchWeather(city) {
  ...
  const celsiusTemp = Math.round(weather.main.feels_like - 273) + '&deg;';

  const markup = `<div class="weather-card"><div class="weather-card__info"><p class="weather-card__temperature">${celsiusTemp}</p><p class="weather-card__main">${weather.weather[0].main}</p><p class="weather-card__geolocation"><svg width="16" height="16" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
...
</svg>${weather.name}</p></div><img class="weather-card__icon" src="http://openweathermap.org/img/wn/${weather.weather[0].icon}@4x.png"><p class="weather-card__date"><br>${currentWeekday}</br>${currentDate}</p></div>`;
 
  /*return document.body.news-gallery.insertAdjacentHTML('afterbegin', markup);*/
...
async function startWeatherWidget() {
  await getWeatherByGeolocation().then(currentCity => {
